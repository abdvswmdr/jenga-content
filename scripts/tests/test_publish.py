"""Tests for publish.py — cross-post CLI (Dev.to)."""
...
class TestStripZennSyntax:
    ...
    def test_details_to_html(self) -> None:
        ...
        result = _strip_zenn_syntax(md)
        assert "<details><summary>Click to expand</summary>" in result
        assert "Content here." in result
        assert "</details>" in result

    def test_no_special_syntax(self) -> None:
        md = "Regular Markdown text"
...
class TestConvertToDevto:
    def test_with_canonical_url(self) -> None:
        ...
        payload = convert_to_devto(article, canonical_url="https://myblog.com/x")
        art = payload["article"]
        assert art["title"] == "Sample Test Article"
        assert art["canonical_url"] == "https://myblog.com/x"
...
class TestParseArticle:
    def test_parse_sample_article(self) -> None:
        article = parse_article(SAMPLE_ARTICLE)
        assert article.title == "Sample Test Article"
        assert "python" in article.topics
        assert "publishing pipeline" in article.body
...
class TestPublishToDevto:
    ...
    def test_success(self) -> None:
        respx.post("https://dev.to/api/articles").mock(
            return_value=httpx.Response(201, json={"url": "https://dev.to/user/article"})
        )
...
class TestUpdateOnDevto:
    ...
    def test_success(self) -> None:
        respx.put("https://dev.to/api/articles/42").mock(
            return_value=httpx.Response(200, json={"url": "https://dev.to/user/article"})
        )
...
class TestFindDevtoArticleByTitle:
    ...
    def test_found(self) -> None:
        respx.get("https://dev.to/api/articles/me/published").mock(
            return_value=httpx.Response(200, json=[{"id": 99, "title": "Target"}])
        )
...
class TestRunDevto:
    ...
    def test_dry_run(self, capsys: pytest.CaptureFixture[str]) -> None:
        article = _make_article()
        args = _make_args(platform="devto", dry_run=True, canonical_url="https://myblog.com")
        ret = _run_devto(article, args)
        assert ret == 0
        out = capsys.readouterr().out
        assert "dry-run" in out
        assert "https://myblog.com" in out
    ...
    def test_update_auto(self, capsys: pytest.CaptureFixture[str]) -> None:
        respx.get("https://dev.to/api/articles/me/published").mock(
            return_value=httpx.Response(200, json=[{"id": 42, "title": "Sample Test Article"}])
        )
        respx.put("https://dev.to/api/articles/42").mock(
            return_value=httpx.Response(200, json={"url": "https://dev.to/user/article"})
        )
...
class TestBuildParser:
    ...
    def test_canonical_url(self) -> None:
        ...
        args = parser.parse_args(
            ["a.md", "--platform", "devto", "--canonical-url", "https://myblog.com"]
        )
        assert args.canonical_url == "https://myblog.com"
